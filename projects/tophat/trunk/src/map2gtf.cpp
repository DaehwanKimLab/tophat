    ...
    gtf_fname_(gtf_fname), reads_fname_(reads_fname), refSeqTable_(true)
{
    ...
    std::cout << "Initializing the BAMHitFactory." << std::endl;
    hitFactory_ = new BAMHitFactory(readTable_, refSeqTable_);
    std::cout << "Done with init samfactory" << std::endl;

    // read bam file
    std::cout << "Initializing the HitStream." << std::endl;
    hitStream_ = new HitStream(reads_fname, hitFactory_, false, true,
			       true, true, true, true);
  ...
  while (hitStream_->next_read_hits(hit_group))
    {
	  ...
	  tokenize(bwt_hit.hitfile_rec().c_str(), "\t", aux_fields);
	  for (size_t j = 11; j < aux_fields.size(); ++j)
	    {
	      const string& aux = aux_fields[j];
	      if (aux.substr(0, 2) == "XS" || aux.substr(0,2) == "NM")
		continue;
...
//out.trans must already have the corresponding GffObj*
{
    ...
    const std::vector<CigarOp>& cigars = in.cigar();
    for (size_t c = 0; c < cigars.size(); ++c)
      {
	const CigarOp& cigar = cigars[c];

	if (cigar.opcode == INS)
	  cig_list.push_back(cigar);
	
	if (cigar.opcode != MATCH && cigar.opcode != DEL)
	  continue;
	...
	for (; i < exon_list.Count(); ++i)
	  {
		...
		cur_pos + remaining_length - 1 <= cur_exon->end) // read ends in this exon
	      {
		CigarOp cig(cigar.opcode, remaining_length);
	    ...
	    else if (cur_pos >= cur_exon->start &&
		     cur_pos + remaining_length - 1 > cur_exon->end)// read is spliced and overlaps this exon
	      {
		spliced = true;
